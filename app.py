...
def get_road_distance(lat1, lon1, lat2, lon2):
    """Calcola distanza stradale reale via OSRM (HTTPS)"""
    try:
...
def get_coords_from_address(address):
    try:
        geolocator = Nominatim(user_agent="comparatore_spesa_v29")
...
user_pos = streamlit_js_eval(js_expressions="navigator.geolocation.getCurrentPosition(pos => pos.coords, err => console.log(err))", key="GPS_AUTO_V29")
...
if user_pos and 'latitude' in user_pos:
    st.session_state.my_lat = user_pos['latitude']
    st.session_state.my_lon = user_pos['longitude']

# --- 4. LOGICA TABS ---
tab_carica, tab_cerca = st.tabs(["📷 CARICA SCONTRINO", "🔍 CERCA PREZZI"])

# --- TAB 1: CARICAMENTO (RIPRISTINATO MULTI-UPLOAD) ---
with tab_carica:
    if 'dati_analizzati' not in st.session_state: st.session_state.dati_analizzati = None
    
    # Ripristinata funzione multi-caricamento e camera trigger
    uploaded_files = st.file_uploader("Carica o scatta foto (anche multiple)", type=['jpg', 'jpeg', 'png'], accept_multiple_files=True)
    ...
    if st.session_state.dati_analizzati:
        ...
        # Match per P.IVA (più robusto dell'indirizzo)
...
# --- TAB 2: RICERCA (LOGICA MATCH P.IVA + INDIRIZZO) ---
with tab_cerca:
    if st.session_state.my_lat:
        st.success(f"📍 GPS Attivo")
    else:
        with st.expander("📍 Imposta posizione (GPS non rilevato)", expanded=True):
            if st.button("Riprova GPS"): st.rerun()
            addr_in = st.text_input("Oppure scrivi la città")
